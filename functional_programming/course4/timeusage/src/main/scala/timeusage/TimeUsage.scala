...
/*
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val grouped = columnNames.groupBy { name =>
      if (name.startsWith("t01") || name.startsWith("t03") || name.startsWith("t11") || name.startsWith("t1801") || name.startsWith("t1803"))
        ...
        || name.startsWith("t09") || name.startsWith("t10") || name.startsWith("t12") || name.startsWith("t13")
        || name.startsWith("t14") || name.startsWith("t15") || name.startsWith("t16") || name.startsWith("t18"))
        "other"
      else
        "skip"
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when($"teage".between(15.0, 22.0), "young")
      .otherwise(when($"teage".between(23, 55), "active").otherwise("elder")).as("age")
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String = {
    s"select working, sex, age, round(avg(primaryNeeds),1) as primaryNeeds, round(avg(work),1) as work, round(avg(other),1) as other from $viewName " +
      s"group by working, age, sex order by working, sex, age"
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age))
      .agg(typed.avg(_.primaryNeeds), typed.avg(_.work), typed.avg(_.other)).map {
