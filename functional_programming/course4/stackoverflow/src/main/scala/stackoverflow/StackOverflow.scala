...
class StackOverflow extends Serializable {
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map { case (question, score) => (firstLangInTag(question.tags, langs), score) }
      .filter(_._1.isDefined).map { case (tagIndex, score) => (tagIndex.get * langSpread, score)
    }.cache()
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    vectors.groupBy(findClosest(_, means)).mapValues(averageVectors(_)).collect().foreach {
      case (i, average) => newMeans.update(i, average)
    }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      val langIndex = vs.groupBy(_._1).map(v => (v._1, v._2.size)).maxBy(_._2)._1
      val langLabel: String = langs(langIndex / langSpread)
      val langPercent: Double = vs.filter(_._1 == langIndex).size * 100d / vs.size
