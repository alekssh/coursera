...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> incomingRanks = sites.join(ranks)
                .flatMapToPair(siteRank -> {
                            Website website = siteRank._2._1;
                            Double outcomingRank = siteRank._2._2 / website.getNEdges();

                            List<Tuple2<Integer, Double>> ranksContributions = new ArrayList<>();

                            for (Iterator iterator = website.edgeIterator(); iterator.hasNext(); ) {

                                ranksContributions.add(new Tuple2<>((Integer) iterator.next(), outcomingRank));
                            }
                            return ranksContributions;
                        }
                );

        return incomingRanks.reduceByKey((rank1, rank2) -> rank1 + rank2).mapValues(v -> 0.15 + 0.85 * v);
    }
